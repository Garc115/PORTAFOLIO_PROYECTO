            ...
            <div class="card shadow p-3 mb-5 bg-body rounded">
                <div class="row justify-content-center" style="margin-top:0em;margin-bottom:0em;padding:0em">
                    <div class="col-12" align="center" style="color:#222222;font-family:Soho Std;font-size:2.2em">
                        Gestión de Consultas (Faltas Administrativas)
                    </div>
                </div>
                <hr />
                <div class="card-body" style="padding:0em 2em 2em 2em">
                    ...
                    <form id="search_by_date_range" method="GET"
                        action="{{ route('consultants.arrested-fault-search-by-date-range') }}">
                            ...
                            <table id="table_info" class="table table-bordered" style="width:100%">
                                ...
                                <tbody>
                                    @foreach ($arrested as $person)
                                        <tr>
                                            @isset($person->generalDataFault)
                                            <th>{{ $person->generalDataFault->code }}</th>
                                            <th>{{ $person->generalDataFault->log_sheet_id }}</th>
                                            <th>{{ $person->generalDataFault->carbon_arrival_date->monthName }}</th>
                                            <th>{{ $person->generalDataFault->arrival_date }}</th>
                                            <th>{{ $person->generalDataFault->carbon_arrival_date->dayName }}</th>
                                            <th>{{ $person->generalDataFault->arrival_hour }}</th>
                                            @else
                                            ...
                                            @isset($person->generalDataFault)
                                            <th>{{ $person->generalDataFault->street }}</th>
                                            <th>{{ $person->generalDataFault->exterior_number }}</th>
                                            <th>{{ $person->generalDataFault->interior_number }}</th>
                                            <th>{{ $person->generalDataFault->postal_code }}</th>
                                            <th>{{ $person->generalDataFault->colony }}</th>
                                            <th>{{ $person->generalDataFault->general_township }}</th>
                                            <th>{{ $person->generalDataFault->general_state }}</th>
                                            @else
                                            <th>Sin datos generales vinculados</th>
                                            <th>Sin datos generales vinculados</th>
                                            <th>Sin datos generales vinculados</th>
                                            ...
                                            <th>Sin datos generales vinculados</th>
                                            @endisset
                                            <th>{{ $person->nationsality }}</th>
                                            <th>{{ $person->firstRespondent->full_name }}</th>
                                            @isset($person->generalDataFault)
                                            <th>{{ $person->generalDataFault->unit_name }}</th>
                                            @else
                                            <th>Sin datos generales vinculados</th>
                                            @endif
                                            @isset($person->firstRespondent)
                                            <th>{{ $person->firstRespondent->institution->institution }}</th>
                                            @else
                                            <th>Sin datos generales vinculados</th>
                                            @endif
                                            <th>@isset($person->generalDataFault->judge->full_name)
                                                {{ $person->generalDataFault->judge->full_name }}
                                            @endisset</th>
                                        </tr>
                                    @endforeach
                                </tbody>
